        ...
        <main class="col py-3">
            ...
            <table width="95%" class="table table-bordered">
              ...
              <?php if (isset($_GET['class'])) {
                        ...
                        $class = $_GET['class'];
                        if($class == "4"){
                            $stmt = $conn->query("SELECT * FROM m4 WHERE ((stat='2' or stat='3') and urole='user') ORDER BY id");
                            $stmt->execute();
                        }
                        else if($class == "1"){
                            $stmt = $conn->query("SELECT * FROM m1 WHERE ((stat='2' or stat='3') and urole='user') ORDER BY id");
                            $stmt->execute();
                        }
                        $users = $stmt->fetchAll();
                      }
